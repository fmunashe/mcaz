...
class ViewQualityProblemReport extends State
{
    ...
    protected function beforeRendering(): void
    {
        ...
        if ($client) {
            $defects = \App\Models\ProductDefect::where('client_id', $client->id)->pluck('report_number')->toArray();
            $message = '';
            foreach ($defects as $index => $defect) {
                $message = $message . ($index + 1) . '. ' . $defect . '\n';
            }

            $this->menu->line('Hello ' . $client->full_name);
            $this->menu->line('Here are your product defects submissions');
