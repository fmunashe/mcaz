...
class ViewAefiReport extends State
{
    ...
    protected function beforeRendering(): void
    {
        ...
        if ($client) {
            $aefis = \App\Models\AEFI::where('client_id', $client->id)->pluck('mcaz_reference_number')->toArray();
            $message = '';
            foreach ($aefis as $index => $aefi) {
                $message = $message . ($index + 1) . '. ' . $aefi . '\n';
            }

            $this->menu->line('Hello ' . $client->full_name);
            $this->menu->line('Here are your AEFI submissions');
